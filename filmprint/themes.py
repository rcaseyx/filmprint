...
_NOISE_KEYWORDS: frozenset[str] = frozenset({
    ...
    "approving", "awestruck", "baffled", "bewildered", "blunt", "bold", "brisk",
    "callous", "celebratory", "clinical", "comforting", "condescending", "confusion",
    "cruel", "cynic", "didactic", "direct", "disdainful", "disheartening", "doubtful",
    "exhilarated", "exuberant", "factual", "familiar", "forceful", "frantic",
    "heartfelt", "hopeful", "indifferent", "intense", "matter of fact", "observer",
    "optimistic", "pathetic", "reflective", "serious", "speculative", "suspenseful",
    "torment", "unassuming", "nostalgia",
    "amused", "amusing", "admiration", "appreciative", "assertion", "assertive",
    "bitter", "bitterness", "bittersweet", "cheerful", "moody", "defiant", "impulsive",
    "depraved", "depressing", "derisive", "phasing", "detached", "emotional detachment",
    "dignified", "superficiality", "blasphemy", "disapproving", "dishonesty", "honesty",
    "dramatic", "savagery", "furious", "vice squad", "gentle", "serene", "gloomy",
    "grim", "hopeless", "hopelessly in love", "inflammatory", "sepsis",
    "informative", "instructive", "introspection", "introspective", "irreverence",
    "irreverent", "light-hearted", "lighthearted", "malice", "malicious", "meditative",
    "stammering", "modest", "modesty", "nostalgic", "nostalgic memory", "pessimism",
    "pessimistic", "powerful", "profound", "preposterous", "pretentious", "escalation",
    "provocative", "relaxed", "slump", "bumbling", "sadistic", "sarcasm", "sarcastic",
    "sentimental", "sincere", "sympathetic", "thoughtful", "understated", "urgent",
    "whimsical", "whump", "foreboding", "bebop", "incredulous", "dread",
...
_NOISE_THEMES: frozenset[str] = frozenset({
    ...
    "Amused", "Appreciative", "Assertive", "Bitter", "Bittersweet", "Cheerful",
    "Defiant", "Depressing", "Derisive", "Detached", "Dignified", "Disapproving",
    "Dishonesty", "Dramatic", "Furious", "Gentle", "Gloomy", "Grim", "Hopeless",
    "Incredulous", "Inflammatory", "Informative", "Introspective", "Irreverent",
    "Foreboding", "Lighthearted", "Malicious", "Meditative", "Modest", "Nostalgic",
    "Pessimistic", "Powerful", "Pretentious", "Provocative", "Relaxed", "Sadistic",
    "Sarcasm", "Sarcastic", "Sentimental", "Sincere", "Sympathetic", "Thoughtful",
    "Understated", "Urgent", "Whimsical", "Approving", "Awestruck", "Baffled",
    "Bewildered", "Blunt", "Bold", "Brisk", "Callous", "Celebratory", "Clinical",
    "Comforting", "Condescending", "Confusion", "Cruel", "Cynic", "Didactic",
    "Direct", "Disdainful", "Disheartening", "Doubtful", "Exhilarated", "Exuberant",
    "Factual", "Familiar", "Forceful", "Frantic", "Heartfelt", "Hopeful",
    "Indifferent", "Intense", "Matter Of Fact", "Observer", "Optimistic", "Pathetic",
    "Reflective", "Serious", "Speculative", "Suspenseful", "Torment", "Unassuming",
    "Nostalgia",
})
...
class _OnnxEncoder:
    def __init__(self, onnx_path: str, tokenizer_path: str) -> None:
        ...
        self._tokenizer = Tokenizer.from_file(os.path.join(tokenizer_path, "tokenizer.json"))
...
def _get_model():
    ...
    if _model is None:
        if _Path(_ONNX_PATH).exists() and _Path(_TOKENIZER_PATH).exists():
            ...
            raise RuntimeError(
                f"ONNX model or tokenizer not found at {_ONNX_PATH} / {_TOKENIZER_PATH}. "
                "Run scripts/export_onnx.py to generate them."
            )
    return _model
...
def build_clusters() -> int:
    ...
    # assign_new_keywords() skips already-assigned keywords (correct for incremental
    # backfill). A full recluster must reassign sparse keywords against fresh
    # centroids so stale assignments from prior runs don't persist.
    ...
    if sparse_to_assign and cluster_centroids:
        ...
        c_norm = np.stack([cluster_centroids[t] for t in theme_names])
        c_norm = c_norm / (np.linalg.norm(c_norm, axis=1, keepdims=True) + 1e-8)
        ...
        sparse_rows = [
            (kw, theme_names[int(np.argmax(sim_row))], "auto")
            for kw, sim_row in zip(sparse_to_assign, sims)
            if sim_row[int(np.argmax(sim_row))] >= ASSIGN_THRESHOLD
            and existing_full.get(kw, {}).get("source") not in ("seed", "claude")
        ]
